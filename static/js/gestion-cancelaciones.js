...
async function cargarSolicitudes() {
    try {
        ...
        solicitudesData.sort((a, b) => {
            const fechaA = new Date(a.fecha_solicitud_cancelacion || 0);
            const fechaB = new Date(b.fecha_solicitud_cancelacion || 0);
...
function mostrarSolicitudes(solicitudes) {
    ...
    container.innerHTML = solicitudes.map(solicitud => {
        const fechaSolicitud = new Date(solicitud.fecha_solicitud_cancelacion);
        ...
        const estadoClass = `status-${solicitud.estado_cancelacion || 'Solicitada'}`;

        return `
            <div class="bg-white rounded-xl shadow-md overflow-hidden border-2 ${solicitud.estado_cancelacion === 'Solicitada' ? 'border-yellow-400' : 'border-gray-200'} hover:shadow-lg transition-all">
                <div class="p-6">
                    <div class="flex flex-col lg:flex-row lg:items-start lg:justify-between gap-4 mb-4">
                        <div class="flex-1">
                            <div class="flex items-center gap-2 mb-3">
                                <span class="status-badge ${estadoClass}">${solicitud.estado_cancelacion || 'Solicitada'}</span>
                        ...
                        <div class="lg:w-1/3">
                            <div class="bg-gray-50 rounded-lg p-4 border-l-4 border-yellow-500">
                                ...
                                <p class="text-sm text-gray-800">${solicitud.motivo_cancelacion}</p>
                            </div>
                        </div>
                    </div>

                    ${solicitud.estado_cancelacion === 'Solicitada' ? `
                        <div class="pt-4 border-t border-gray-200 flex gap-3">
                            ...
                            <button onclick="procesarSolicitud(${solicitud.id_reserva}, 'Cancelada')" 
                    ...
                    ` : `
                        <div class="pt-4 border-t border-gray-200">
                            <p class="text-sm text-gray-600">
                                ${solicitud.estado_cancelacion === 'Cancelada' ? '✅ Esta solicitud fue aprobada y la reserva está cancelada.' : '❌ Esta solicitud fue rechazada y la reserva sigue activa.'}
                            </p>
                        </div>
                    `}
                </div>
...
function aplicarFiltros() {
    ...
    if (estado) {
        solicitudesFiltradas = solicitudesFiltradas.filter(s => 
            (s.estado_cancelacion || 'Solicitada') === estado
        );
    }
    ...
    if (fechaDesde) {
        solicitudesFiltradas = solicitudesFiltradas.filter(s => {
            const fechaSolicitud = new Date(s.fecha_solicitud_cancelacion);
...
function limpiarFiltros() {
    document.getElementById('filtroEstado').value = 'Solicitada';
...
async function procesarSolicitud(idReserva, nuevoEstado) {
    const accion = nuevoEstado === 'Cancelada' ? 'aprobar' : 'rechazar';
    const confirmacion = confirm(`¿Estás seguro de que deseas ${accion} esta solicitud de cancelación?`);
    ...
    try {
        const response = await fetch('/reservas/api/trabajador/procesar-cancelacion', {
            ...
            body: JSON.stringify({
                ...
                estado_cancelacion: nuevoEstado
            })
        });
