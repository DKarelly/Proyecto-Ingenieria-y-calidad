// Generar Reportes de Ocupación de Recursos - JavaScript
document.addEventListener('DOMContentLoaded', function() {
    // Variables globales
    let recursosCompletos = [];
    let recursosFiltrados = [];
    let reportesOcupacion = [];
    let tiposRecurso = [];
    let paginaActual = 1;
    ...
    filtroTipoRecurso.addEventListener('change', aplicarFiltros);
    filtroBuscarRecurso.addEventListener('input', aplicarFiltros);
    ...
    // Función para cargar tipos de recursos
    async function cargarTiposRecurso() {
        try {
            const response = await fetch('/reportes/api/tipos-recurso');
            ...
            mostrarNotificacion('Error al cargar tipos de recursos', 'error');
        }
    }

    // Función para generar reporte
    async function generarReporte() {
        const tipoRecurso = filtroTipoRecurso.value || null;
        ...
        try {
            const response = await fetch('/reportes/api/generar-ocupacion', {
                method: 'POST',
                headers: {
            ...
            if (data.success) {
                reportesOcupacion = data.data.reportes;
                recursosCompletos = data.data.reportes;
                ...
                mostrarModalExito(data.data.total);
            } else {
                mostrarNotificacion(data.message || 'Error al generar reporte', 'error');
            }
        } catch (error) {
            console.error('Error:', error);
            mostrarNotificacion('Error al generar reporte de ocupación', 'error');
        } finally {
    ...
    function mostrarModalExito(total) {
        ...
        document.getElementById('exitoFecha').textContent = fechaActual;
    ...
    function aplicarFiltros() {
        const tipoRecurso = filtroTipoRecurso.value;
        const busqueda = filtroBuscarRecurso.value.toLowerCase().trim();

        recursosFiltrados = recursosCompletos.filter(recurso => {
            const matchTipo = !tipoRecurso || recurso.idTipoRecurso == tipoRecurso;
            const matchBusqueda = !busqueda || recurso.nombreRecurso.toLowerCase().includes(busqueda);
    ...
    window.verDetalle = function(idRecurso) {
        ...
        document.getElementById('detalleTipo').textContent = recurso.tipoRecurso;
        ...
        document.getElementById('detallePeriodo').textContent = recurso.periodo;
        document.getElementById('detallePorcentaje').textContent = recurso.porcentajeOcupacion;
        document.getElementById('detalleDisponibilidad').textContent = recurso.disponibilidad;
        document.getElementById('detalleHorasUso').textContent = `${recurso.horasUso} horas`;
        document.getElementById('detalleHorasDisponibles').textContent = `${recurso.horasDisponibles} horas`;
        document.getElementById('detalleFecha').textContent = formatearFechaHora(recurso.fechaGeneracion);
    ...
    function exportarReporte(formato) {
        mostrarNotificacion(`Exportando reporte de ocupación en formato ${formato.toUpperCase()}...`, 'success');
        modalExportar.style.display = 'none';
        
        // TODO: Implementar exportación real
        setTimeout(() => {
            mostrarNotificacion(`Reporte de ocupación exportado exitosamente`, 'success');
        }, 1500);
    }
