            ...
            try:
                cursor.execute("""
                    SELECT id_examen, fecha_examen, 
                           TIME_FORMAT(hora_examen, '%%H:%%i') as hora_examen,
                           observacion, estado
                    FROM EXAMEN
                    WHERE id_reserva = %s
                """, (id_reserva,))
                ...
                # Si falla (columna hora_examen no existe), intentar sin ella
                ...
                try:
                    ...
                    if examen:
                        if examen.get('fecha_examen'):
                            examen['fecha_examen'] = examen['fecha_examen'].strftime('%Y-%m-%d')
                        examen['hora_examen'] = 'No disponible'
                        reserva['examen'] = examen
        ...
        with conexion.cursor() as cursor:
            ...
            cursor.execute("""
                ...
                SET estado = 'Cancelada',
                    ...
                    hora_cancelada = %s
                WHERE id_reserva = %s
            """, (fecha_actual, hora_actual, id_reserva))
        ...
        try:
            with conexion_tipo.cursor() as cursor:
                ...
                if 'consulta' in tipo_servicio_nombre or 'cita' in tipo_servicio_nombre:
                    ...
                    # Crear EXAMEN
                    cursor.execute("""
                        INSERT INTO EXAMEN (fecha_examen, hora_examen, estado, id_reserva)
...
def paciente_crear_reserva():
    ...
    try:
        ...
        # Verificar si ya hay una programación ocupada para este horario
        conexion = obtener_conexion()
        with conexion.cursor() as cursor:
            # Verificar si el horario ya tiene una programación ocupada
            sql_check = """
                ...
                AND estado = 'Ocupado'
            """
            cursor.execute(sql_check, (int(id_horario),))
            result = cursor.fetchone()
        ...
        with conexion.cursor() as cursor:
            ...
            if id_servicio:
                sql_prog = """
                    ...
                    VALUES (%s, %s, %s, %s, %s, %s)
                """
                cursor.execute(sql_prog, (fecha, hora_inicio, hora_fin, 'Ocupado', id_servicio, id_horario))
            else:
                ...
                sql_prog = """
                    ...
                    VALUES (%s, %s, %s, %s, %s)
                """
                cursor.execute(sql_prog, (fecha, hora_inicio, hora_fin, 'Ocupado', id_horario))
    ...
    try:
        ...
        with conexion.cursor() as cursor:
            ...
            sql_horario = """
                ...
                LIMIT 1
            """
            cursor.execute(sql_horario, (hora_examen, hora_examen, fecha_examen, hora_examen, hora_examen))
