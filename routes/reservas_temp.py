...
# ==================== ENDPOINTS DE CANCELACIÃ“N Y REPROGRAMACIÃ“N ====================
...
def api_solicitar_cancelacion():
    """API para solicitar la cancelaciÃ³n de una reserva"""
    if 'usuario_id' not in session:
        return jsonify({'error': 'No autenticado'}), 401
    ...
    if not motivo or len(motivo.strip()) < 10:
        return jsonify({'error': 'Debe proporcionar un motivo vÃ¡lido (mÃ­nimo 10 caracteres)'}), 400
    ...
    try:
        ...
        import traceback
        traceback.print_exc()
        return jsonify({'error': f'Error al solicitar cancelaciÃ³n: {str(e)}'}), 500
    finally:
        if conexion:
            try:
                conexion.close()
